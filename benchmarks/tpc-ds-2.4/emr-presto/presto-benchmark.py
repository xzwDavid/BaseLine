...
results_dir = "results/presto/iceberg/timestamp=%d" % (int(time.time()))
    ...
    for num_run in xrange(num_runs):
        for filename in files:
            ...
            run_command("echo \"explain \" > __tmp_get_query_plan.sql && " +
                        ...
                        (query_dir, filename))
            run_command("presto-cli --catalog iceberg " +
                        "--server localhost:8081 --schema %s --file __tmp_get_query_plan.sql > %s/plans/%s.plan" %
                        (databaseName, results_dir, filename))
            # Run the query
            print "Running " + filename
            run_command("""cat __tmp_presto_configs.sql %s/%s > __tmp_current_query.sql""" %
                        (query_dir, filename))
            run_command("presto-cli --catalog iceberg --schema %s --file __tmp_current_query.sql --server localhost:8081 "
                        "> %s/runs/%s.run 2>&1" % (databaseName, results_dir, filename))
            # Get query runtime
            run_command("presto-cli --catalog iceberg --schema %s --file --server localhost:8081"
